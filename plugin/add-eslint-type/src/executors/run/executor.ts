...
export default async function runExecutor(options: RunExecutorSchema) {
  ...
  function updateProjectLint(repositories) {
    ...
    for (const repositoryInfo of repositories) {
      const { id, created_at, last_activity_at, name, web_url } =
        repositoryInfo;
  ...
  function getDirectories(directoryPath) {
    try {
      ...
      directories = directories
        .filter((v) => !blackFileList.includes(v))
  ...
  function readPackageJson(directoryPath) {
    ...
    if (packageJson.dependencies) {
      if (
        ...
        packageJson.dependencies['react-native-webview'] ||
        packageJson.dependencies.umi ||
        packageJson.dependencies['@umijs/max']
      ) {
        pkg_type = 'react-';
    ...
    if (packageJson.devDependencies) {
      if (
        ...
        packageJson.devDependencies.preact ||
        packageJson.devDependencies['react-native-webview'] ||
        packageJson.devDependencies.umi ||
        packageJson.devDependencies['@umijs/max']
      ) {
        pkg_type = 'react-';
  ...
  function getLintTypeByPath(directoryPath) {
    ...
    final_lint_type = fs.existsSync(tsconfigFilePath)
      ? pkg_type + 'ts'
      : pkg_type + 'js';
  ...
  function detectProjectType(directoryPath, rootName) {
    ...
    const rootHasPnpmWorkSpace = fs.existsSync(
      path.join(directoryPath, 'pnpm-workspace.yaml')
    );
    ...
    if (rootHasPnpmWorkSpace) {
      ...
      const pnpmWorkSpacePath = findFilePath(
        directoryPath,
        'pnpm-workspace.yaml'
      );
      ...
      const pnYmalReg = /\/\*{0,2}$/;
      const pnpmPkgs = pnpmWorkSpaceContent.packages.map((v) =>
        v.replace(pnYmalReg, '')
      );
      ...
      pnpmPkgs.forEach((rootFile) => {
        ...
        if (fs.existsSync(temPath)) {
          ...
          fileNames.forEach((fileName) => {
            ...
            const res = !isDirectoryEmptySync(filePath)
              ? getLintTypeByPath(filePath)
              : '';
            typeList[`${rootFile}`] = {
              ...typeList[`${rootFile}`],
              [`${fileName}`]: res,
  ...
  async function checkLintType() {
    ...
    console.log(`项目共有 ${allProjects.length} 个 \n`);
